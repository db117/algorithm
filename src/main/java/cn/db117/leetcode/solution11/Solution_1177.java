...
// 
//
// Related Topics 位运算 数组 哈希表 字符串 前缀和 👍 123 👎 0
 ...
 *
 * @author db117
 * @since 2023-06-15 11:28:37
 **/

public class Solution_1177 {
    public static void main(String[] args) {
        Solution solution = new Solution_1177().new Solution();
    }
    ...
    class Solution {
        public List<Boolean> canMakePaliQueries(String s, int[][] queries) {
            ...
            // 每个字符出现的次数进行前缀和
            int[][] pre = new int[26][chars.length + 1];
            ...
            for (int[] query : queries) {
                ...
                for (int i = 0; i < 26; i++) {
                    // 当前字符出现的次数
                    int count = pre[i][query[1] + 1] - pre[i][query[0]];
                    // 出现奇数次
                    if ((count & 1) == 1) {
                        cur++;
                    }
