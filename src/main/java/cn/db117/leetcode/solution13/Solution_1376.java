 ...
 *
 * @author db117
 * @since 2023-05-01 23:14:13
 **/

public class Solution_1376 {
    public static void main(String[] args) {
        Solution solution = new Solution_1376().new Solution();
    }
    ...
    class Solution {
        public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
            ...
            while (!queue.isEmpty()) {
                ...
                int nextTime = informTime[index];

                List<Integer> nexts = map.getOrDefault(index, Collections.emptyList());
                for (Integer next : nexts) {
                    queue.add(new int[]{next, time + nextTime});
                }
            }
            return ans;
        }
