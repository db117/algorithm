...
// 
//
// Related Topics 数组 动态规划 排序 👍 164 👎 0
 ...
 *
 * @author db117
 * @since 2024-11-12 14:03:29
 **/

public class Solution_1547 {
    public static void main(String[] args) {
        Solution solution = new Solution_1547().new Solution();
    }
    ...
    class Solution {
        ...
        private int dfs(int[] cuts, int start, int end) {
            ...
            for (int i = start + 1; i < end; i++) {
                int cost = dfs(cuts, start, i) + dfs(cuts, i, end);
                ans = Math.min(ans, cost);
            }

            return memo[start][end] = ans + (cuts[end] - cuts[start])/*当前切割的成本*/;
        }
    }
