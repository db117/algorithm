 ...
 *
 * @author db117
 * @since 2023-03-05 22:54:21
 **/

public class Solution_1599 {
    public static void main(String[] args) {
        Solution solution = new Solution_1599().new Solution();
    }
    ...
    class Solution {
        public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
            ...
            for (int i = 0; i < customers.length || earth > 0; i++) {
                if (i < customers.length) {// 客户还在来的情况下
                    earth += customers[i];
                }

                if (earth >= 4) {// 一次最多 4 个人
                    fly += 4;
                    earth -= 4;
                } else {
                    fly += earth;
                ...
                int cur = fly * boardingCost - runningCost * (i + 1);
                if (cur > max) {
                    max = cur;
                    ans = i + 1;
                }
