 ...
 *
 * @author db117
 * @since 2024-03-28 23:29:31
 **/

public class Solution_1997 {
    public static void main(String[] args) {
        Solution solution = new Solution_1997().new Solution();
    }

    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public int firstDayBeenInAllRooms(int[] nextVisit) {
            // 0 <= nextVisit[i] <= i  需要往前走，到达当前位置只能是 i-1 偶数次的情况
            int n = nextVisit.length;
            int mod = (int) 1e9 + 7;
            int[] dp = new int[n];
            for (int i = 1; i < n; i++) {
                // 2 次到达 i-1 的次数  - 1 次到达 nextVisit[i-1] 的次数 + 2
