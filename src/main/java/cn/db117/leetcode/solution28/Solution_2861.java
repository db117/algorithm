 ...
 *
 * @author db117
 * @since 2023-09-18 11:22:13
 **/

public class Solution_2861 {
    public static void main(String[] args) {
        Solution solution = new Solution_2861().new Solution();
    }
    ...
    class Solution {
        public int maxNumberOfAlloys(int n, int k, int budget, List<List<Integer>> composition, List<Integer> stock, List<Integer> cost) {
            ...
            for (int i = 0; i < k; i++) {
                ...
                while (left < right) {
                    ...
                    if (check(n, budget, integers, stock, cost, mid)) {
                        left = mid;
                    } else {
                        right = mid - 1;
                    }
        ...
        public boolean check(int n, long budget, List<Integer> composition, List<Integer> stock, List<Integer> cost, int target) {
            for (int i = 0; i < n; i++) {
                if (stock.get(i) > (long) composition.get(i) * target) {
                    continue;
                }

                long need = (long) composition.get(i) * target - stock.get(i);
