...
// 
//
// Related Topics 深度优先搜索 广度优先搜索 并查集 图 数组 哈希表 有序集合 堆（优先队列） 👍 8 👎 0
 ...
 *
 * @author db117
 * @since 2025-08-12 19:06:38
 **/

public class Solution_3607 {
    public static void main(String[] args) {
        Solution solution = new Solution_3607().new Solution();
    }
    ...
    class Solution {
        public int[] processQueries(int c, int[][] connections, int[][] queries) {
            ...
            for (int i = 1; i <= c; i++) {
                map.putIfAbsent(uf.find(i - 1), new TreeSet<>());
                // 把每个节点加入到集合中，key 是集合最小的节点
                map.get(uf.find(i - 1)).add(i);
            }

            Set<Integer> flag = new HashSet<>();
            List<Integer> answer = new ArrayList<>();
            for (int[] query : queries) {
                TreeSet<Integer> set = map.get(uf.find(query[1] - 1));
                if (query[0] == 1) {
                    if (set.isEmpty()) {
                        answer.add(-1);
                    } else {
                        if (flag.contains(query[1])) {
                            // 集群中最小的节点
