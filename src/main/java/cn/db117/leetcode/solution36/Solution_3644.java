...
// 解释： 
//
// 选择 k = 1。交换 nums[1] = 3 和 nums[3] = 1，因为 nums[1] AND nums[3] == 1，从而得到一个排序后的排
//列：[0, 1, 2, 3]。 
//
// 示例 2： 
//
...
// 解释： 
//
// 选择 k = 2。交换 nums[2] = 3 和 nums[3] = 2，因为 nums[2] AND nums[3] == 2，从而得到一个排序后的排
//列：[0, 1, 2, 3]。 
//
// 示例 3： 
//
 ...
 *
 * @author db117
 * @since 2025-08-11 15:51:50
 **/

public class Solution_3644 {
    public static void main(String[] args) {
        Solution solution = new Solution_3644().new Solution();
    }
    ...
    class Solution {
        public int sortPermutation(int[] nums) {
            int ans = -1;
            // 找需要交换的元素，取他们的 & 值
            // 如果其中某一个数字可以和其他所有数字交换，那么就都可以调整
            for (int i = 0; i < nums.length; i++) {
                if (nums[i] != i) {// 当前元素需要交换
                    ans &= nums[i];
                }
                if (ans == 0) {
                    break;
