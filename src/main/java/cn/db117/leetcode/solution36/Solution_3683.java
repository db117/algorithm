...
// 解释： 
//
// 第一个任务从时间 t = 1 开始，并在 1 + 6 = 7 时完成。第二个任务在时间 t = 2 开始，并在 2 + 3 = 5 时完成。因此，最早完成
//的任务在时间 5。 
//
// 示例 2： 
//
 ...
 *
 * @author db117
 * @since 2025-09-26 15:50:42
 **/

public class Solution_3683 {
    public static void main(String[] args) {
        Solution solution = new Solution_3683().new Solution();
    }

    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public int earliestTime(int[][] tasks) {
            int ans = tasks[0][1] + tasks[0][0];
            for (int[] task : tasks) {
                int start = task[0];
                int end = task[1];
