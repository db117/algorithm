...
function test(input: number[], expected: number): void {
    const actual = minimumDistance(input);
    if (actual !== expected) {
        throw new Error(`FAIL input=${JSON.stringify(input)} expected=${expected} actual=${actual}`);
    }
    console.log(`PASS input=${JSON.stringify(input)} result=${actual}`);
}
