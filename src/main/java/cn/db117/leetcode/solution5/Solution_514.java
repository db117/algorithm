 ...
 *
 * @author db117
 * @since 2024-01-29 11:20:29
 **/

public class Solution_514 {
    public static void main(String[] args) {
        Solution solution = new Solution_514().new Solution();
    }
    ...
    class Solution {
        public int findRotateSteps(String ring, String key) {
            ...
            boolean[][] visited = new boolean[k.length][s.length];
            int step = 0;
            ...
            while (!deque.isEmpty()) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (s[start] == k[index]) {
                        ...
                        deque.offer(new int[]{index + 1, start});
                        continue;
                    }


                    // right
                    int right = (start + 1) % s.length;
                    if (!visited[index][right]) {
                        visited[index][right] = true;
                        deque.offer(new int[]{index, right});
                    }

                    // left
                    int left = (start - 1 + s.length) % s.length;
                    if (!visited[index][left]) {
                        visited[index][left] = true;
                        deque.offer(new int[]{index, left});
                    }
                }
                step++;
            }
