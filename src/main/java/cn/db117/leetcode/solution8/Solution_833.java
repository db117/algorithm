...
// 
//
// Related Topics 数组 字符串 排序 👍 117 👎 0
 ...
 *
 * @author db117
 * @since 2023-08-15 10:36:12
 **/

public class Solution_833 {
    public static void main(String[] args) {
        Solution solution = new Solution_833().new Solution();
    ...
    class Solution {
        public String findReplaceString(String s, int[] indices, String[] sources, String[] targets) {
            ...
            for (int i = 0; i < n; i++) {
                // 从后往前替换，不会影响前面的索引
                int index = arr[i][0];// 原始字符串的索引
                int j = arr[i][1];
                String source = sources[j];
                String target = targets[j];
                if (s.startsWith(source, index)) {// 找到了
